...
def load_warcraft_3_model(model, importProperties):
    ...
    model.source_dir = str(Path(getattr(importProperties, 'mdx_file_path', '.')).parent)
    model.team_color = importProperties.set_team_color
    ...
    create_object_actions(model, bpyObjects, importProperties.frame_time, armatureObject)
    if getattr(importProperties, 'import_effects', True) and model.effects:
        from .effects import create_effects
        create_effects(model, armatureObject, importProperties)
    if model.import_warnings:
        armatureObject['warcraft_3_warnings'] = '\n'.join(model.import_warnings)
    ...
    # Selecting the initial entry also assigns its Action and Slot.
    armatureObject.data.warcraft_3.sequencesListIndex = 1 if model.sequences else 0
...
def create_mesh_objects(model, setTeamColor):
    ...
    from .textures import load_images
    model.normalize_meshes_names()
    bpyImages = load_images(model, preferences, setTeamColor)
    model.blender_images = bpyImages
    bpyMaterials = []
    for material in model.materials:
        ...
        if bpyImagesOfLayer:
            ...
            alpha = bpyMaterial.node_tree.nodes.new('ShaderNodeMath')
            alpha.operation = 'MULTIPLY'
            bpyMaterial.node_tree.links.new(texImage.outputs['Alpha'], alpha.inputs[0])
            bpyMaterial.node_tree.links.new(objInfo.outputs['Alpha'], alpha.inputs[1])
            bpyMaterial.node_tree.links.new(alpha.outputs[0], bsdf.inputs['Alpha'])

        bpyMaterials.append(bpyMaterial)
    model.blender_materials = bpyMaterials
    bpyObjects = []
...
def create_armature_object(model, bpyObjects, boneSize):
    ...
    for indexNode, node in enumerate(nodes):
        nodePosition = pivotPoints[node.node.id]
        ...
        bonesById[node.node.id] = bone
        boneTypes[bone.name] = node.type
        # Keep Blender's actual (possibly disambiguated) name for F-curve paths.
        node.node.blender_bone_name = bone.name
    nodeTypes = list(nodeTypes)
    nodeTypes.sort()
    for indexNode, node in enumerate(nodes):
        bone = bonesById[node.node.id]
        if node.node.parent is not None and node.node.parent in bonesById:
            parent = bonesById[node.node.parent]
            bone.parent = parent
    for mesh in bpyObjects:
        ...
        for vertexGroup in mesh.vertex_groups:
            ...
            boneName = bonesById[vertexGroupIndex].name
...
def create_armature_actions(armatureObject, model, frameTime):
    ...
    entry = add_sequence_to_armature('#UNANIMATED', armatureObject)
    fcurves = create_action_fcurves('#UNANIMATED', armatureObject, entry)
    for node in nodes:
        boneName = node.node.blender_bone_name
        dataPath = armatureObject.pose.bones[boneName].path_from_id()
    ...
    for sequence in sequences:
        ...
        entry = add_sequence_to_armature(sequence.name, armatureObject)
        fcurves = create_action_fcurves(sequence.name, armatureObject, entry)
        entry.action.use_frame_range = True
        entry.action.frame_start = 0
        entry.action.frame_end = max(1, (intervalEnd - intervalStart) / frameTime)
        for node in nodes:
            boneName = node.node.blender_bone_name
            dataPath = armatureObject.pose.bones[boneName].path_from_id()
                ...
                for index in range(translations.tracks_count):
                    ...
                    if intervalStart <= time and time <= intervalEnd:
                        ...
                        if not locationFcurveZ:
                            locationFcurveZ = fcurves.new(data_path=dataPath + '.location', index=2)
                        realTime = (time - intervalStart) / frameTime
            ...
            if scalings:
                ...
                if not scaleFcurveZ:
                    scaleFcurveZ = fcurves.new(data_path=dataPath + '.scale', index=2)
                    scaleFcurveZ.keyframe_points.insert(0.0, 1.0)
            # Every sequence defines a complete pose, including untracked channels.
            for property_name, default in (('location', 0.0), ('rotation_euler', 0.0), ('scale', 1.0)):
                for axis in range(3):
                    path = dataPath + '.' + property_name
                    if fcurves.find(path, index=axis) is None:
                        curve = fcurves.new(data_path=path, index=axis)
                        curve.keyframe_points.insert(0.0, default)



def create_object_actions(model, bpyObjects, frameTime, armatureObject):
    geosetAnimations = model.geoset_animations
    sequences = model.sequences
    dataPathColor = 'color'
    for geosetAnimation in geosetAnimations:
        geosetId = geosetAnimation.geoset_id
        fcurves = create_action_fcurves('#UNANIMATED' + ' ' + bpyObjects[geosetId].name, bpyObjects[geosetId], armatureObject.data.warcraft_3.sequencesList[0])
    ...
    for sequenceIndex, sequence in enumerate(sequences, 1):
        intervalStart = sequence.interval_start
        intervalEnd = sequence.interval_end
        for geosetAnimation in geosetAnimations:
            geosetId = geosetAnimation.geoset_id
            ...
            fcurves = create_action_fcurves(sequence.name + ' ' + bpyObjects[geosetId].name, bpyObjects[geosetId], armatureObject.data.warcraft_3.sequencesList[sequenceIndex])
            colorR = None
            colorG = None
            colorB = None
            colorA = None

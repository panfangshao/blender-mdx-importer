...
def parse_layers(data):
    ...
    for _ in range(layersCount):
        ...
        layer.filter_mode = filterMode
        shadingFlags = r.getf('<I')[0]
        layer.shading_flags = shadingFlags
...
def parse_sequences(data, model):
    ...
    for _ in range(sequenceCount):
        ...
        flags = r.getf('<I')[0]
        sequence.non_looping = bool(flags & 1)
        rarity = r.getf('<f')[0]
...
def parse_mdx(data, importProperties):
    ...
    while r.offset < dataSize:
        chunkId = r.getid(constants.SUB_CHUNKS_MDX_MODEL + ('GLBS', 'PRE2', 'RIBB', 'PREM'), debug=True)
        ...
        if chunkId == constants.CHUNK_VERSION:
            ...
            model.global_sequences = list(binary.Reader(chunkData).getf('<%dI' % (chunkSize // 4)))
        elif chunkId in ('PRE2', 'RIBB'):
            emitters = effects_data.read_emitters(chunkData, binary.Reader, parse_node,
                                                 'particle' if chunkId == 'PRE2' else 'ribbon')
            model.effects.extend(emitters)
            model.nodes.extend(emitters)
        elif chunkId == 'PREM' and chunkSize:
            model.import_warnings.append('Legacy PREM model particles are not supported.')
    return importer.load_warcraft_3_model(model, importProperties)
...
def load_mdx(importProperties):
    ...
    mdxFile.close()
    return parse_mdx(mdxFileData, importProperties)
